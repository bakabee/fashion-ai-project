    ...
    const ctx = gsap.context(() => {
      ...
      itemsRef.current.forEach((el, i) => {
        gsap.from(el, {
          ...
          y: 150 + i * 40,
          opacity: 0,
          rotation: collageItems[i].rotate * 2,
          duration: 1.5,
        });
  ...
  return (
    ...
    >
          ...
          >
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/30 to-transparent p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <p className="text-white text-[9px] tracking-[0.15em] uppercase font-medium">
                  {item.label}
                </p>
              </div>
              {/* Pin */}
              <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-dark-300 shadow-sm border border-white/50" />
            </motion.div>
          </div>
        ))}
      </div>
      ...
      <div className="relative z-40 text-center px-6 max-w-2xl">
        <p
          ref={textRef}
          className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-dark-800 tracking-tight leading-tight"
        >
          Inspiration begins
          <br />
        ...
        <p
          ref={descRef}
          className="mt-4 text-sm md:text-base text-dark-400 font-light max-w-md mx-auto"
        >
          Every collection starts with a line on paper. Explore curated sketches, structural studies, and editorial concepts.
        </p>
        <div className="mt-8 flex items-center justify-center gap-2 text-[10px] tracking-[0.2em] uppercase text-dark-300">
          <span className="w-8 h-px bg-dark-200" />
